...
from .knowledge_mining import search_index, view_result


@login_required
def index(request):
    return render(request, 'knowledge_mining/index.html', {})
...
def search(request):
    if request.htmx:
        if not str(request.POST['query']) == "":
            ...
            return render(request, 'knowledge_mining/_results.html', context)
        return render(request, 'knowledge_mining/_results.html', {})
...
def view(request, key_id=None):
    ...
    return render(request, 'knowledge_mining/view.html', {
        'result': result,
